...
function doGet(e){
  ...
  try{
    ...
    // Reports: Bundle + Lite rows (single call, faster initial page load)
    if(action === "reportBundleLite"){
      const auth = requireRole(p, "ADMIN");
      if(!auth.ok) return jsonp(auth, callback);
      p.__session = auth.session;
      return jsonp(reportBundleLite(p), callback);
    }
    ...
    // Reports: Lite payload (faster initial page load)
    if(action === "reportPunchesLite"){
      const auth = requireRole(p, "ADMIN");
      if(!auth.ok) return jsonp(auth, callback);
      p.__session = auth.session;
      return jsonp(reportPunchesLite(p), callback);
    }
        if(action === "listLogs"){
      const auth = requireRole(p, "SUPER_ADMIN");
      if(!auth.ok) return jsonp(auth, callback);
...
/**
 * Optimized Reports endpoint: returns summary + lite punch rows in ONE call.
 * Goal: speed up initial Reports page load.
 * Returns per row: punch_date, volunteer_id, punched_at, group
 *
 * Notes:
 * - Uses chunked reads from the bottom of Punches to reduce work for recent date ranges.
 * - Assumes Punches are appended chronologically (newest at bottom). Set p.safe=1 to disable early-stop.
 */
function reportBundleLite(p){
  const from = String(p.from || "").trim();
  const to = String(p.to || "").trim();
  const groupFilter = String(p.group || "").trim();
  const safe = String(p.safe || "") === "1";
  if(!from || !to) return { ok:false, error:"MISSING_PARAMS" };
  ...
  // Ensure group header exists (backward compat)
  ensureHeader(shP, "group");
  ensureHeader(shV, "group");

  // ---- Volunteers: totals + groupById (light read) ----
  const vLastCol = Math.max(1, shV.getLastColumn());
  const vHeader = shV.getRange(1,1,1,vLastCol).getValues()[0].map(h=>norm(h));
  const vIdx = {};
  vHeader.forEach((h,i)=>{ if(h) vIdx[h]=i; });
  if(vIdx.id === undefined) return { ok:false, error:"VOL_HEADER_ID_MISSING" };

  const vLastRow = shV.getLastRow();
  const vCount = Math.max(0, vLastRow - 1);
  const groupById = {};
  let totalAll = 0;
  const totalByGroup = { all: 0 };

  if(vCount > 0){
    const idCol = vIdx.id + 1;
    const gCol0 = (vIdx.group !== undefined ? vIdx.group : vIdx.groupe);
    const gCol = (gCol0 !== undefined) ? (gCol0 + 1) : null;

    const ids = shV.getRange(2, idCol, vCount, 1).getValues();
    const gs = gCol ? shV.getRange(2, gCol, vCount, 1).getValues() : null;

    for(let i=0;i<vCount;i++){
      const id = String(ids[i][0]||"").trim();
      if(!id) continue;
      const g = gs ? String(gs[i][0]||"").trim() : "";
      groupById[id] = g;
      totalAll++;
      const key = norm(g) || "(SANS)";
      totalByGroup[key] = (totalByGroup[key]||0)+1;
  ...
  totalByGroup.all = totalAll;

  const totalVolunteers = (groupFilter && norm(groupFilter) !== "ALL")
    ? (totalByGroup[norm(groupFilter)] || 0)
    : (totalByGroup.all || 0);

  // ---- Punches: chunked scan ----
  const pLastCol = Math.max(1, shP.getLastColumn());
  const pHeader = shP.getRange(1,1,1,pLastCol).getValues()[0].map(h=>norm(h));
  const pIdx = {};
  pHeader.forEach((h,i)=>{ if(h) pIdx[h]=i; });

  // Required columns
  const need = {
    punch_date: pIdx.punch_date,
    volunteer_id: pIdx.volunteer_id,
    punched_at: pIdx.punched_at,
    group: (pIdx.group !== undefined ? pIdx.group : pIdx.groupe)
  };
  if(need.punch_date === undefined || need.volunteer_id === undefined || need.punched_at === undefined){
    return { ok:false, error:"PUNCH_HEADERS_MISSING" };
  }

  const minCol0 = Math.min(need.punch_date, need.volunteer_id, need.punched_at, (need.group!==undefined?need.group:need.punch_date));
  const maxCol0 = Math.max(need.punch_date, need.volunteer_id, need.punched_at, (need.group!==undefined?need.group:need.punched_at));
  const colStart = minCol0 + 1;
  const colWidth = (maxCol0 - minCol0) + 1;
  const off = {
    punch_date: need.punch_date - minCol0,
    volunteer_id: need.volunteer_id - minCol0,
    punched_at: need.punched_at - minCol0,
    group: (need.group!==undefined ? (need.group - minCol0) : null)
  };
  ...
  while(end >= 2){
    ...
    const block = shP.getRange(start, colStart, n, colWidth).getValues();

    // Iterate from bottom to top (recent to old)
    for(let i=n-1;i>=0;i--){
      const r = block[i];
      const d = toYMD(r[off.punch_date]);
      if(!d) continue;

      // If sheet is chronological and we're below from-date, we can stop early
      if(!safe && d < from){
        end = 1; // will exit outer while
        break;
      }
      if(d > to) continue;
      if(d < from) continue;

      const vid = String(r[off.volunteer_id]||"").trim();
      if(!vid) continue;

      let g = "";
      if(off.group !== null) g = String(r[off.group]||"").trim();
      if(!g) g = String(groupById[vid] || "");

      if(groupFilter && norm(groupFilter) !== "ALL" && norm(g) !== norm(groupFilter)) continue;

      const rowObj = {
        punch_date: d,
        volunteer_id: vid,
        punched_at: toISO(r[off.punched_at]),
        group: g
      };
      out.push(rowObj);
      totalPunches++;
  ...
  // Sort by date then time (asc) to keep existing UI behavior
  out.sort((a,b)=>{
  ...
  const uniqueVolunteers = Object.keys(uniq).length;
...
/**
 * Same as reportPunches but with a much smaller payload.
 * Used to speed up Reports initial load.
 * Fields returned per row: punch_date, volunteer_id, punched_at, group
 */
function reportPunchesLite(p){
  const from = String(p.from || "").trim();
  const to = String(p.to || "").trim();
  const group = String(p.group || "").trim();
  if(!from || !to) return { ok:false, error:"MISSING_PARAMS" };
